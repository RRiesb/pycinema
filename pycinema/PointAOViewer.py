...
class PointAOViewer(Filter):
    ...
    def __init__(self, path):
        ...
        self.shaderPointAO.inputs.images.set(self.colorMapping.outputs.images,False)
        self.shadingWidgetsContainer.children = [
          
             # TODO: change the parameters below to parameters we actually need
            NumberWidget(self.shaderPointAO.inputs.radius, range=[0.1,10,0.1]).widget,
            NumberWidget(self.shaderPointAO.inputs.samples, range=[1,128,1]).widget,
            NumberWidget(self.shaderPointAO.inputs.scalers, range=[1,20,1]).widget,
            NumberWidget(self.shaderPointAO.inputs.densityweight, range=[0.001,2,0.001]).widget,
            NumberWidget(self.shaderPointAO.inputs.totalStrength, range=[0.0,5,0.1]).widget,
